...
class ChatClientGUI:
    ...
    def send_file(self) -> None:
        ...
        try:
            ...
            # Show progress (без отображения base64)
            file_size_kb = len(file_data) / 1024
            self.log(f"📎 Sending file: {file_name_only} ({file_size_kb:.1f} KB)...")

            # Send file message
            self.sock.sendall(file_msg.encode('utf-8'))
    ...
    def _receiver_loop(self) -> None:
        ...
        try:
            ...
            self.incoming_queue.put(f"Error: {str(e)}")
        finally:
                ...
                try:
                    ...
                    if len(parts) == 4:
                        ...
                        if save_path:
                            ...
                            file_size_kb = len(file_data) / 1024
                            self.log(f"📎 File '{filename}' from {sender} saved ({file_size_kb:.1f} KB)")
                        else:
